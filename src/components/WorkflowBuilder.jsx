...
// import { ItemComponents } from "./components/ItemComponents.jsx";
...
function WorkflowBuilder() {
  ...
  const rearrangeItems = (items) => {
    ...
    const maxItemsPerRow = Math.max(
      1,
      Math.floor(
        (containerWidth - horizontalMargin) / (itemWidth + horizontalSpacing)
      )
    );
    ...
    if (selectedItem) {
      setWorkspaceItems(
        workspaceItems.map((item) =>
          item.id === selectedItem.id ? { ...item, hasNote: true, note } : item
        )
      );
      setShowModal(false);
    }
  ...
  const handleRemoveNote = (itemId) => {
    setWorkspaceItems(
      workspaceItems.map((item) =>
        item.id === itemId ? { ...item, hasNote: false, note: "" } : item
      )
    );
  };
  ...
  return (
    <div className="container-fluid vh-100 d-flex flex-column">
      <div className="row flex-grow-1">
        <div
          className="col-3 p-0 border-end"
        ...
        <div className="col-9 p-0 d-flex flex-column">
          ...
          />
          <ItemComponents.SelectedElements
            items={workspaceItems}
            onRemoveItem={handleRemoveItem}
          />
        </div>
